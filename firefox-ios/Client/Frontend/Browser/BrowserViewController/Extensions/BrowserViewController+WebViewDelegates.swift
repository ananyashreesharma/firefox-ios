    ...
    func webView(
        ...
        if let url = error.userInfo[NSURLErrorFailingURLErrorKey] as? URL {
            ...
            var queryItems: [URLQueryItem] = [
                ...
                URLQueryItem(
                    name: NativeErrorPageHelper.Constants.codeQueryParam,
                    value: String(error.code)
                )
            ]

            queryItems.append(contentsOf: NativeErrorPageHelper.certErrorQueryItems(from: error))
            ...
            if let errorPageURL = errorPageURLComponents.url {
                ...
                if isNoInternetError || isCertificateError {
                    if isCertificateError {
                        NativeErrorPageHelper.logCertificateError(
                            error: error,
                            url: url,
